...
CREATE TABLE IF NOT EXISTS `senicare`.`nurses` (
  ...
  `password` VARCHAR(255) NOT NULL COMMENT '요양사 암호화된 비밀번호',
...
CREATE TABLE IF NOT EXISTS `senicare`.`customers` (
  ...
  `profile_image` TEXT NOT NULL COMMENT '고객 사진',
  ...
  `location` VARCHAR(60) NOT NULL COMMENT '고객 지역 (시, 도)',
...
CREATE TABLE IF NOT EXISTS `senicare`.`tools` (
  ...
  `name` VARCHAR(255) NOT NULL COMMENT '용품명(용품 이름)',
  `purpose` TEXT NOT NULL COMMENT '용품 용도',
  `count` INT NOT NULL COMMENT '용품 잔여 수량 (개수)',
...
CREATE TABLE IF NOT EXISTS `senicare`.`care_records` (
  ...
  `count` INT NULL COMMENT '관리 시 사용한 용품 개수',
